...
public class HomeGameManager : GameManager
{
    ...
    private void Update()
    {
        // 소지 골드와 소지 용액의 개수를 갱신함
        gold_text.text =  gold.ToString();
        solution_text.text = solution_num.ToString();

        // 상점의 유형을 구매와 판매로 나뉘어서 활성화 되도록 함
    ...
    // 다른 Scene을 불러옴
    public void ChangeScene(string type) { SceneManager.LoadScene(type); }

    // 모든 아이템 또는 용액의 추가 금액을 결정하고 적용하는 메소드
    ...
    // 아이템 또는 용액의 추가 금액을 해당 아이템 또는 용액의 금액의 1/10 ~ 1/2의 가격까지로 설정하는 메소드
